...
# 基于自己笔记本的一些默认参数
# 记录训练结果的文档
log_path = r'D:\CV\CV论文调研\MyResnet_COCO\version_3\result_log.txt'
#  数据集根路径
...
eval_step = 1
# val acc最高的模型权重保存路径
best_model = r'D:\CV\CV论文调研\MyResnet_COCO\version_3\checkppoints\best_resnet.pkl'
# initial_checkpoint
initial_checkpoint = r'D:\CV\CV论文调研\MyResnet_COCO\version_3\checkppoints\resnet18-f37072fd.pth'
# 训练权重文件的保存路径
output_dir = r'D:\CV\CV论文调研\MyResnet_COCO\version_3\output'
#resume checkpoint路径
...
def train_model(model, criterion, optimizer, scheduler, num_epochs, use_gpu, eval_step,
    ...
    for epoch in range(resume_epoch ,num_epochs):
        ...
        print('Train Loss: {:.4f} Acc: {:.4f} lr: {:.8f}'.format(epoch_loss, epoch_acc, optimizer.param_groups[0]['lr']))

        with open(log_path, 'a+') as f:
            print('Train Loss: {:.4f} Acc: {:.4f} lr: {:.8f}'.format(epoch_loss, epoch_acc, optimizer.param_groups[0]['lr']), file=f)
        
        #plot train curve 
        #plot(loss=epoch_loss, acc=epoch_acc, epoch= epoch, is_train=True)

        plot_lr(epoch, optimizer.param_groups[0]['lr'])
